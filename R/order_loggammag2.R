...
#' @return A list with a random sample of order statistics from a Log Gamma G II Distribution, the value of its join probability density function evaluated in the random sample
...
#' order_loggammag2(10,"exp",1,1,k=3,n=50,p=0.5,alpha=0.02)
#' # A sample of size 10 of the 3-th order statistics from
#' # a Log Gamma Normal II Distribution
#' order_loggammag2(10,"norm",1,1,k=3,n=50,p=0.5)
#' # A sample of size 10 of the 3-th order statistics from
#' # a Log Gamma Log-normal II Distribution
#' order_loggammag2(10,"lnorm",1,1,k=3,n=50,p=0.5)
#' # A sample of size 10 of the 3-th order statistics from
#' # a Log Gamma Chi-square II Distribution
#' order_loggammag2(10,"chisq",1,1,k=3,n=50,p=0.5,df=3)
#' @importFrom Newdistns qloggammag2 dloggammag2
#' @importFrom stats rbeta
#' @export order_loggammag2

order_loggammag2 <- function(size,spec,a,b,k,n,p=0.5,alpha=0.05,...){
  sample  <- qloggammag2(initial_order(size,k,n),spec,a,b,...)
  pdf     <- factorial(size)*cumprod(dloggammag2(sample,spec,a,b,...))[size]
  log_pdf     <- sum(log(2:size)) + sum(log(dloggammag2(sample,spec,a,b,...)))
  if(size>5){
