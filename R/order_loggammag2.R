#' Random Sampling of Order Statistics from a Log Gamma G II Distribution
#'
#'\code{order_loggammag2} is used to obtain a random sample of order statistics from a Log Gamma G II Distribution.
...
#' @return A list with a random sample of order statistics from a Log Gamma G II Distribution and the value of its join probability density function evaluated in the random sample.
...
#' order_loggammag2(10,"exp",1,1,k=3,50)
#' # A sample of size 10 of the 3-th order statistics from
...
order_loggammag2 <- function(size,spec,a,b,k,n,...){
  initial <- rbeta(size, k, n + 1 - k)
  sample  <- qloggammag2(initial,spec,a,b,...)
