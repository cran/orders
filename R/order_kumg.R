#' Random Sampling of Order Statistics from a Kumaraswamy G Distribution
#'
#'\code{order_expkumg} is used to obtain a random sample of order statistics from a Kumaraswamy G Distribution.
...
#' @return A list with a random sample of order statistics from a Kumaraswamy G Distribution and the value of its join probability density function evaluated in the random sample.
...
#' order_kumg(10,"exp",1,1,k=3,50)
#' # A sample of size 10 of the 3-th order statistics from
#' # a Kumaraswamy Normal Distribution
#' order_kumg(10,"norm",1,1,k=3,50)
#' # A sample of size 10 of the 3-th order statistics from
#' # a Kumaraswamy Log-normal Distribution
#' order_kumg(10,"lnorm",1,1,k=3,50)
#' # A sample of size 10 of the 3-th order statistics from
#' # a Kumaraswamy Chi-square Distribution
#' order_kumg(10,"chisq",1,1,k=3,50,df=3)
#' @importFrom Newdistns qkumg dkumg
#' @importFrom stats rbeta
#' @export order_kumg

order_kumg <- function(size,spec,a,b,k,n,...){
  initial <- rbeta(size, k, n + 1 - k)
  sample  <- qkumg(initial,spec,a,b,...)
