#' Random Sampling of k-th Order Statistics from a Exponentiated G Distribution
#'
#'\code{order_expg} is used to obtain a random sample of the k-th order statistic from a Exponentiated G Distribution.
...
#' @return A list with a random sample of order statistics from a Exponentiated G Distribution, the value of its join probability density function evaluated in the random sample
...
#' order_expg(10,"exp",1,k=3,50,alpha=0.02)
#' # A sample of size 10 of the 3-th order statistics from a Exponentiated Normal Distribution
#' order_expg(10,"norm",1,k=3,50)
#' # A sample of size 10 of the 3-th order statistics from a Exponentiated Log-normal Distribution
#' order_expg(10,"lnorm",1,k=3,50)
#' # A sample of size 10 of the 3-th order statistics from a Exponentiated Chi-square Distribution
#' order_expg(10,"chisq",1,k=3,50,df=3)
#' @importFrom Newdistns qexpg dexpg
#' @importFrom stats rbeta
#' @export order_expg

order_expg <- function(size,spec,a,k,n,alpha=0.05,...){
  sample  <- qexpg(initial_order(size,k,n),spec,a,...)
  pdf     <- factorial(size)*cumprod(dexpg(sample,spec,a,...))[size]
  if(size>5){
