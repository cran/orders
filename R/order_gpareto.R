#' Random Sampling of k-th Order Statistics from a Generalized Pareto Distribution
#'
...
#' @param ... represents others parameters of a Generalized Pareto distribution.
#' @return A list with a random sample of order statistics from a Generalized Pareto Distribution, the value of its join probability density function evaluated in the random sample
...
#' order_gpareto(size=10,shape1=0.75,shape2=1,scale=0.5,k=3,n=50,p=0.5,alpha=0.02)
#' @importFrom actuar qgenpareto dgenpareto
#' @importFrom stats rbeta
#' @export order_gpareto

order_gpareto <- function(size,k,shape1,shape2,scale,n,p=0.5,alpha=0.05,...){
  sample  <- qgenpareto(initial_order(size,k,n),shape1,shape2,scale,...)
  pdf     <- factorial(size)*cumprod(dgenpareto(sample,shape1,shape2,scale,...))[size]
  log_pdf     <- sum(log(2:size)) + sum(log(dgenpareto(sample,shape1,shape2,scale,...)))
  if(size>5){
