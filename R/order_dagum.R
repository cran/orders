...
#' @return A list with a random sample of order statistics from a Dagum Distribution, the value of its join probability density function evaluated in the random sample
...
order_dagum <- function(size,k,shape1,shape2,scale,n,p=0.5,alpha=0.05,...){
  ...
  log_pdf     <- sum(log(2:size)) + sum(log(ddagum(sample,shape1,shape2,scale,...)))
  if(size>5){
