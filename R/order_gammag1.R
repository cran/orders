#' Random Sampling of Order Statistics from a Gamma G I Distribution
#'
...
#' @return A list with a random sample of order statistics from a Gamma G I Distribution and the value of its join probability density function evaluated in the random sample.
...
order_gammag1 <- function(size,spec,a,k,n,...){
  initial <- rbeta(size, k, n + 1 - k)
  sample  <- qgammag1(initial,spec,a,...)
