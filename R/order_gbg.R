...
#' @return A list with a random sample of order statistics from a Generalized Beta G Distribution, the value of its join probability density function evaluated in the random sample
...
#' order_gbg(10,"norm",1,1,1,k=3,n=50,p=0.5)
#' # A sample of size 10 of the 3-th order statistics from
#' # a Generalized Beta Log-normal Distribution
#' order_gbg(10,"lnorm",1,1,1,k=3,n=50,p=0.5)
#' # A sample of size 10 of the 3-th order statistics from
#' # a Generalized  Beta Chi-square Distribution
#' order_gbg(10,"chisq",1,1,1,k=3,n=50,p=0.5,df=3)
#' @importFrom Newdistns qgbg dgbg
#' @importFrom stats rbeta
#' @export order_gbg

order_gbg <- function(size,spec,a,b,c,k,n,p=0.5,alpha=0.05,...){
  sample       <- qgbg(initial_order(size,k,n),spec,a,b,c,...)
  pdf          <- factorial(size)*cumprod(dgbg(sample,spec,...))[size]
  log_pdf     <- sum(log(2:size)) + sum(log(dgbg(sample,spec,...)))
  if(size>5){
