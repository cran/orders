...
#' and an approximate (1 - alpha) confidence interval for the population percentile p of the k-th order statistic.
...
#' #order_zip(size=10,k=5,mu=5,sigma=0.1,n=30,p=0.5)
#' @importFrom gamlss.dist qZIP dZIP
#' @importFrom stats rbeta

order_zip <- function(size,k,mu,sigma,n,p=0.5,alpha=0.05,...){
  sample  <- qZIP(initial_order(size,k,n),mu,sigma,...)
  pdf     <- factorial(size)*cumprod(dZIP(sample,mu,sigma,...))[size]
  log_pdf     <- sum(log(2:size)) + sum(log(dZIP(sample,mu,sigma,...)))
  if(size>5){
