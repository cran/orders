#' Random Sampling of k-th Order Statistics from a Exponentiated Generalized G Distribution
#'
#'\code{order_eg} is used to obtain a random sample of k-th order order statistic from a Exponentiated Generalized G Distribution.
...
#' @return A list with a random sample of order statistics from a Exponentiated Generalized G Distribution, the value of its join probability density function evaluated in the random sample
...
#' # a Exponentiated Generalized Exponential Distribution
#' order_eg(10,"exp",1,1,k=3,n=50,p=0.5,alpha=0.02)
#' # A sample of size 10 of the 3-th order statistics from
#' # a Exponentiated Generalized Normal Distribution
#' order_eg(10,"norm",1,1,k=3,n=50,p=0.5)
#' # A sample of size 10 of the 3-th order statistics from
#' # a Exponentiated Generalized Log-normal Distribution
#' order_eg(10,"lnorm",1,1,k=3,n=50,p=0.5)
#' # A sample of size 10 of the 3-th order statistics from
#' # a Exponentiated Generalized Chi-square Distribution
#' order_eg(10,"chisq",1,1,k=3,n=50,p=0.5,df=3)
#' @importFrom Newdistns qeg deg
#' @importFrom stats rbeta
#' @export order_eg

order_eg <- function(size,spec,a,b,k,n,p=0.5,alpha=0.05,...){
  sample  <- qeg(initial_order(size,k,n),spec,a,b,...)
  pdf     <- factorial(size)*cumprod(deg(sample,spec,a,b,...))[size]
  log_pdf     <- sum(log(2:size)) + sum(log(deg(sample,spec,a,b,...)))
  if(size>5){
