#' Random Sampling of k-th Order Statistics from a Beta Extended G Distribution
#'
...
#' @return A list with a random sample of order statistics from a Beta Extended G Distribution, the value of its join probability density function evaluated in the random sample
...
#' order_beg(10,"norm",1,1,k=3,n=50,p=0.5)
#' # A sample of size 10 of the 3-th order statistics from a Beta Extented Log-normal Distribution
#' order_beg(10,"lnorm",1,1,k=3,n=50,p=0.5)
#' # A sample of size 10 of the 3-th order statistics from a Beta Extented Chis-square Distribution
#' order_beg(10,"chisq",1,1,k=3,n=50,p=0.5,df=3)
#' @importFrom Newdistns qbeg dbeg
#' @importFrom stats rbeta
#' @export order_beg

order_beg <- function(size,spec,a,b,k,n,p=0.5,alpha=0.05,...){
  sample  <- qbetaexpg(initial_order(size,k,n),spec,a,b,...)
  pdf     <- factorial(size)*cumprod(dbetaexpg(sample,spec,a,b,...))[size]
  log_pdf     <- sum(log(2:size)) + sum(log(dbetaexpg(sample,spec,a,b,...)))
  if(size>5){
